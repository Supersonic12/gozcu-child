...
void ConfigHandler::loadFile()
{
    ...
    {
        std::cerr << "config directory doesn't exists." << getConfigPath() << std::endl;
    }
...
std::map<std::string, bool> ConfigHandler::getMaskMap()
{
    std::map<std::string, bool> maskStateMap;
    const std::string maskKey = "markMask";
    maskStateMap = get<std::map<std::string, bool>>(maskKey);
    return maskStateMap;
}
...
{
    set<std::map<std::string, bool>>("markMask", maskMap);
}
