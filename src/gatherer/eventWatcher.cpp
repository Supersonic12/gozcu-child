...
void eventWatcher::startWatching()
{
     ...
     * It doesnt interfere with processes file access. It gives us Which file is affected and event
     * data in fanotify_event_metadata struct
     */
    int fd = fanotify_init(
        FAN_CLASS_NOTIF | FAN_REPORT_DIR_FID | FAN_REPORT_NAME | FAN_REPORT_FID | FAN_NONBLOCK,
    ...
    //  though i will need to remove this pollfd struct. It basically serves no purpose since this
    //  was supposed to be an agent like application no cli based one
    nfds_t nfds = 2;
    struct pollfd fds[2];
    ...
    for (const auto entry : watchedPaths_)
    {
        ...
        /*
         * this basically sets which events should be recorded and i have basically set all notify
         * events as mask this mask is needed as argument of startWatching function
         */
        int mark = fanotify_mark(fd, FAN_MARK_ADD,
    ...
    /*
     * event loop of watcher. it basically never stops until enter pressed or fanotify error
     * happened it sets fanotify_event_metadata struct and iterates in it to get needed data about
     * event..
     */
    while (1)
    {
        ...
        if (poll_num > 0)
        {
            ...
            if (fds[1].revents & POLLIN)
            {
                 ...
                 */
                struct fanotify_event_metadata* metadata = (struct fanotify_event_metadata*)ptr;
                 ...
                 * here we set event structs variables. though I don't really understand this code.
                 */
                while (FAN_EVENT_OK(metadata, remainingBufLen))
                {
                    ...
                    {
                        if (((char*)infoHeader) + sizeof(struct fanotify_event_info_header) >
                            event_end)
                            break;
                        if (infoHeader->info_type == FAN_EVENT_INFO_TYPE_DFID_NAME)
                        {
                            ...
                            /*
                             * and info_fid struct is nested in infoHeaderso we just create a fid
                             * struct and take file handle from it to get affected files path.
                             */
                            struct fanotify_event_info_fid* fid =
                                (struct fanotify_event_info_fid*)infoHeader;
                            struct file_handle* fh = (struct file_handle*)fid->handle;
                            // here setEventData takes fanotify_init file descriptor, roots file
                            // descriptor, info_fid struct's file_handle and already setted event
                            // structure.
                        ...
                        infoHeader = (struct fanotify_event_info_header*)((char*)infoHeader +
                                                                          infoHeader->len);
                    }
...
bool eventWatcher::setEventData(int fd, int mount_fd, struct file_handle* fh, eventData& event)
{
    // this fd is for reopening another file descriptor to affected file and the reading its path
    // using readlink and fd file in /proc/self/fd. This only works on linux if i ever think of
    // using this kind of mechanism in another OS I will need to reimplement startWatching and
    // setEventData completely. (i guess that means eventWatcher class completely.)
    int refd = open_by_handle_at(mount_fd, fh, O_RDONLY | O_PATH | O_NONBLOCK);
    // i have written and fdguard which basically closes fd when it isn't used or out of block
